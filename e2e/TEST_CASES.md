...
| TC-R07 | 미래 날짜 시간 선택       | clock 15:00 + 미래일 선택 | 미래일은 영업시간(11:00~18:00) **전체** 노출     | ❌ Fail |

> ⚠️ TC-R04의 통과는 픽스처가 **'오늘 · 영업 시작(11:00) 전'** 으로 시각을 고정했기 때문입니다. 미래 날짜 경로(TC-R07)를 추가하자 아래 결함이 드러났습니다.
...
> 🔍 **발견된 결함 (시간 슬롯 필터)**: `TimeSelect`가 선택된 날짜를 전달받지 않고(`date` prop 부재), 항상 **현재 시각** 기준으로 과거 슬롯을 제거합니다(`TimeSelect.tsx:49-58`, 의존성에 선택 날짜 없음).
> 그 결과 **미래 날짜를 선택해도 '현재 시각 이전' 슬롯이 사라집니다**(예: 오늘 15:00 → 미래일에 11:00~14:30 누락, TC-R07). 빈 경우 문구도 `"오늘 선택 가능한 시간이 없습니다"`로 잘못 노출됩니다.
> → '과거 시각 제거' 필터는 **선택 날짜가 오늘일 때만** 적용해야 합니다.
